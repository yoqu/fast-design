...
**If `MINIMAX_API_KEY` is not configured in the environment, skip generated media for this run**: use honest labelled placeholders (solid token-colored blocks with a short caption, or `—`) instead of generated assets, note the substitution to the user, and treat every 'generated assets' mandate in Phases 3–5 and the quality gates as waived for media assets only. All other quality gates still apply.
...
3. Execute via scripts, save to project — do NOT proceed to Phase 5 until all assets are saved locally (unless the no-API-key fallback above is active)
...
### Phase 5: Build UI
Scaffold the project and build each section following Design and Motion rules. Integrate generated assets and copy. All `<img>`, `<video>`, `<source>`, and CSS `background-image` MUST reference local assets from Phase 3 (unless the no-API-key fallback above is active).
...
- [ ] **No placeholder URLs** — grep the output for `unsplash`, `picsum`, `placeholder`, `placehold`, `via.placeholder`, `lorem.space`, `dummyimage`. If ANY found, STOP and replace with generated assets before delivering (unless the no-API-key fallback above is active).

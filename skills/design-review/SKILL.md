...
**Full rule + worked examples + Hold/dependency semantics:** see the gstack repo (`https://github.com/garrytan/gstack`) for `docs/askuserquestion-split.md` when N>4.

**Non-ASCII characters — write directly, never \u-escape.** When any string
field contains Chinese (繁體/簡體), Japanese, Korean, or other non-ASCII text,
emit the literal UTF-8 characters; never escape them as `\uXXXX` (the pipe is
UTF-8 native, and manual escaping miscodes long CJK strings). Only `\n`,
`\t`, `\"`, `\\` remain allowed. Full rationale + worked example: see the gstack repo (`https://github.com/garrytan/gstack`) for `docs/askuserquestion-cjk.md` when a question contains CJK.
